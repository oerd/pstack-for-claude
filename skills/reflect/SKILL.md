...
The parent finds its own transcript file before fanning out. Derive the directory from the working directory: `~/.claude/projects/<slug>/`, where `<slug>` is the absolute path with every "/" turned into "-", the leading one included (`/Users/you/proj` becomes `-Users-you-proj`). Do not glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.

```bash
proj="$HOME/.claude/projects/$(pwd | tr / -)"
ls -t "$proj"/*.jsonl "$proj"/*/subagents/*.jsonl 2>/dev/null | head -10
```

Two transcript layouts: the session itself (`<id>.jsonl`) and its subagents (`<id>/subagents/<child>.jsonl`).
