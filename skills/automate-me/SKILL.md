...
Locate this workspace's transcripts before fanning out. They live in `~/.claude/projects/$(pwd | tr / -)/`. Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
