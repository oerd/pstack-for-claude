...
- Never message an agent just to check on it. That costs it a turn and disturbs work in flight. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, a background agent's status via `/tasks`, `ListAgents`, or `TaskOutput`. Transcript mtime is not liveness.
