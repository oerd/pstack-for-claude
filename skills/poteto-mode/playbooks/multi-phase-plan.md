...
- [ ] On the operator's go, write the program goal to `GOAL.md` in the run directory with this exact text, and re-read it each tick. "<The plan path, the PR ids in order, the verification rule, who merges, and the done condition.>"
...
- [ ] Arm the 30-minute audit tick. In a local session, a real terminal `/loop`. In a cloud root, a scheduled routine (`/schedule`). Never leave the cadence to memory.
- [ ] Use this tick prompt, verbatim. "Re-read the execution playbook from trunk and GOAL.md. Audit the operation against both and fix drift in this tick. Probe every active lane and judge progress by side effects only. Stand down a stuck lane and dispatch its replacement now. Then post a status message to the operator in chat, whether or not anything changed, with the queue table of PR, owner, state, and head SHA, the verdicts since the last tick, what merged, open operator gates, and blockers."
...
Each live lane runs in its own isolation at the PR head: `isolation: "remote"` where the account has it, otherwise `isolation: "worktree"`. Drive through `control-ui` or `control-cli`.
