...
Transcripts live at `~/.claude/projects/<slug>/<session-uuid>.jsonl`, where `<slug>` is the workspace path with every "/" turned into "-", the leading one included (so `/Users/you/proj` becomes `-Users-you-proj`). Subagent transcripts sit under `<session-uuid>/subagents/`. Every line is one chat message.
